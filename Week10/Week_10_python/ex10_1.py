...
p_y_1 = p_y.T@P_k # complete the line 
    ...
    for y_2 in range(nemission):
        ...
        for x_1 in range(nstates):
            for x_2 in range(nstates):
                P = P + p_y[x_2, y_2]*P_ij[x_1, x_2]*p_y[x_1, y_1]*P_k[x_1] 
                # e.g. p(y1,y2)=p(y2|x2)p(x2|x1)p(y1|x1)p(x1)
                # and sum for all possible x1 and x2 values (marginalization)
        p_y1_y2[y_1, y_2] = P
...
alpha[:, 0] = 0 # complete the line 
# implement formula 16.43
for i in range(1, len(y)):
    alpha[:, i] = 0 # complete the line 
